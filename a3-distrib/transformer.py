...
class Transformer(nn.Module):
    ...
    def forward(self, indices):
        ...
        for layer in self.layers:
            layer_output, attn_map = layer(layer_output)
            attn_maps.append(attn_map)

        final_output = self.output_layer(layer_output)

        log_probs = nn.functional.log_softmax(final_output, dim=-1)
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, input_vecs):
        ...
        S = torch.matmul(Q, K.transpose(-2, -1)) / np.sqrt(K.size(-1))
        attn_map = self.softmax(S)
        attnout = torch.matmul(attn_map, V)

        # input_vecs = input_vecs + self.WO(attnout)
        input_vecs = input_vecs + attnout

        ffnout = self.ffn(input_vecs)

        output = input_vecs + ffnout
...
def train_classifier(args, train, dev):
    all_indices = np.concatenate([ex.input_indexed for ex in train] + [ex.input_indexed for ex in dev])
    vocab_size = int(np.max(all_indices) + 1)
